...
//#define FLOATING_POINT_DEBUG
   ...
   !defined(WITH_DSOUND) && !defined(WITH_SDL3)
...
namespace SoLoud
{
	...
	{		
		...
		if (!inited &&
			(aBackend == Soloud::DIRECTSOUND ||
			aBackend == Soloud::AUTO))
		{
			if (aBufferSize == Soloud::AUTO) buffersize = 4096;
			if (aSamplerate == Soloud::AUTO) samplerate = 48000;

			int ret = dsound_init(this, aFlags, samplerate, buffersize, aChannels);
			if (ret == 0)
			{
				inited = 1;
				mBackendID = Soloud::DIRECTSOUND;
			}
